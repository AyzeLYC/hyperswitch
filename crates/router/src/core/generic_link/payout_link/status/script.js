...
function boot() {
  ...
  if (typeof payoutDetails.return_url === "string") {
    ...
    // Form query params
    var queryParams = {payout_id: payoutDetails.payout_id, status: payoutDetails.status},
    
        url = new URL(payoutDetails.return_url),
...
function renderStatusDetails(payoutDetails) {
  ...
  var statusCardNode = document.getElementById("status-card"),
      merchantHeaderNode = document.getElementById("merchant-header");

  if (typeof payoutDetails.merchant_name === "string" && merchantHeaderNode instanceof HTMLDivElement) {
  ...
  if (typeof payoutDetails.logo === "string" && merchantHeaderNode instanceof HTMLDivElement) {
  ...
  var status = payoutDetails.status,
      statusInfo = {statusImageSrc: "https://live.hyperswitch.io/payment-link-assets/success.png", statusText: "{{i18n_success_text}}", statusMessage: "{{i18n_success_message}}"};
  ...
  switch (status) {
    ...
    case "pending":
      ...
      statusInfo.statusImageSrc = "https://live.hyperswitch.io/payment-link-assets/pending.png";
    ...
    default:
      ...
      statusInfo.statusImageSrc = "https://live.hyperswitch.io/payment-link-assets/failed.png";
  ...
  var statusImageNode = document.createElement("img"),
      ...
      statusMsgNode = document.createElement("div");
  ...
  var resourceInfo = {"{{i18n_ref_id_text}}": payoutDetails.payout_id};
  ...
  for (var key in resourceInfo) {
    ...
    var infoNode = document.createElement("div"),
        infoKeyNode = document.createElement("div"),
        infoValNode = document.createElement("div");
  ...
  for (var i = 0; i <= timeout; i++) {
    ...
    setTimeout(function () {
      ...
      var secondsLeft = timeout - j++,
          innerText = secondsLeft === 0 ? "{{i18n_redirecting_text}}" : `{{i18n_redirecting_in_text}} ${secondsLeft} {{i18n_seconds_text}}`;
      ...
      if (secondsLeft === 0) {
        ...
        setTimeout(function () {
          ...
          try {
            ...
            console.error(`CRITICAL ERROR\nFailed to redirect top document.\nError - ${error}`);
