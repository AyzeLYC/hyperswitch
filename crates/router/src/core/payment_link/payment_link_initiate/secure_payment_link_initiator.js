  ...
  **/
  function initializeSDK() {
    ...
    var encodedPaymentDetails = window.__PAYMENT_DETAILS,
        ...
        sdkUiRules = paymentDetails.sdk_ui_rules,
        labelType = paymentDetails.payment_form_label_type,
        colorIconCardCvcError = paymentDetails.color_icon_card_cvc_error,
    ...
    /* @ts-ignore */
    hyper = window.Hyper(pub_key, { isPreloadEnabled: false, /* TODO: Remove in next deployment */shouldUseTopRedirection: true, redirectionFlags: { shouldRemoveBeforeUnloadEvents: true, shouldUseTopRedirection: true } });
    
    /* @ts-ignore  */
    widgets = hyper.widgets({ appearance: appearance, clientSecret: clientSecret, locale: paymentDetails.locale });
    
    var type = paymentDetails.sdk_layout === "spaced_accordion" || paymentDetails.sdk_layout === "accordion" ? "accordion" : paymentDetails.sdk_layout;

    var enableSavedPaymentMethod = paymentDetails.enabled_saved_payment_method,
        hideCardNicknameField = paymentDetails.hide_card_nickname_field,
      ...
      customMessageForCardTerms: paymentDetails.custom_message_for_card_terms,
        ...
        showCardTerms = paymentDetails.show_card_terms;
    
    if (showCardTerms !== null && typeof showCardTerms === "string") {
      
      unifiedCheckoutOptions.terms = { card: showCardTerms };
  ...
  **/
  function redirectToStatus(paymentDetails) {
    ...
    */
    ...
    let returnUrl = `${window.location.origin}/${arr.join("/")}?locale=${paymentDetails.locale}`;
    ...
    try {
      ...
      var message = { message: "CRITICAL ERROR - Failed to redirect top document. Falling back to redirecting using window.location" reason: error.message },
      var log = { message, url, paymentId, merchantId, attemptId, connector };
