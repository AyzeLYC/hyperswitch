...
function invertToBW(color, bw, asArr) {
  ...
  var DEFAULT_BW = {black: "#090302", white: "#FFFFFC", threshold: Math.sqrt(1.05 * 0.05) - 0.05},
      options = bw === true ? DEFAULT_BW : Object.assign({}, DEFAULT_BW, bw);
  
  return getLuminance(color) > options.threshold ? asArr ? hexToRgbArray(options.black) : options.black : asArr ? hexToRgbArray(options.white) : options.white;
...
/* @ts-ignore */
window.state = {prevHeight: window.innerHeight, prevWidth: window.innerWidth, isMobileView: window.innerWidth <= 1199, currentScreen: "payment_link"};
...
var encodedPaymentDetails = window.__PAYMENT_DETAILS,
    paymentDetails = decodeUri(encodedPaymentDetails),
    pub_key = paymentDetails.pub_key,
    hyper = null;
...
try {
  ...
  /* If parent's window object is restricted, DOMException is thrown which concludes that the webpage is iframed*/
...
function emitPaymentStatus(paymentDetails) {
  ...
  var message = { payment: { status: paymentDetails.status } };
...
function boot() {
  ...
  if (paymentDetails.display_sdk_only) {
    ...
    if (orderDetails !== null) {
      ...
      orderDetails.push({"amount": (paymentDetails.amount - charges).toFixed(2),
...
function initializeEventListeners(paymentDetails) {
  ...
  var primaryColor = paymentDetails.theme,
      lighterColor = adjustLightness(primaryColor, 1.4),
      darkerColor = adjustLightness(primaryColor, 0.8),
      contrastBWColor = invert(primaryColor, true),
      a = lighterColor.match(/[fF]/gi),
      contrastingTone = Array.isArray(a) && a.length > 4 ? darkerColor : lighterColor,
      hyperCheckoutNode = document.getElementById("hyper-checkout-payment"),
      hyperCheckoutCartImageNode = document.getElementById("hyper-checkout-cart-image"),
      hyperCheckoutFooterNode = document.getElementById("hyper-checkout-payment-footer"),
      submitButtonNode = document.getElementById("submit"),
      submitButtonLoaderNode = document.getElementById("submit-spinner");
  ...
  var payNowButtonText = document.createElement("div"),
      payNowButtonText = document.getElementById('submit-button-text'),
      capture_type = paymentDetails.capture_method;
...
function handleSubmit(e, paymentDetails) {
  ...
  hyper.confirmPayment({ widgets: widgets, confirmParams: { /* Make sure to change this to your payment completion page */return_url: paymentDetails.return_url } }).then(function (result) {
      ...
      if (error) {
        ...
        /* Form query params */
        var queryParams = { payment_id: paymentDetails.payment_id, status: result.status },
            url = new URL(paymentDetails.return_url),
...
function formatDate(date) {
  ...
  var locale = navigator.language || navigator.userLanguage,
      timezoneShorthand = date.toLocaleDateString(locale, { day: "2-digit", timeZoneName: "long" }).substring(4).split(" ").reduce(function (tz, c) {
        
      return tz + c.charAt(0).toUpperCase();
        
    }, "");

  var formatted = `${hours}:${minutes} ${suffix} ${timezoneShorthand}  ${month} ${day}, ${year}`;
...
function renderPaymentDetails(paymentDetails) {
  ...
  var priceNode = document.createElement("div"),
      merchantNameNode = document.createElement("div"),
      paymentIdNode = document.createElement("div"),
      merchantLogoNode = document.createElement("img"),
      paymentExpiryNode = document.createElement("div");
  ...
  var paymentMerchantDetails = document.getElementById("hyper-checkout-payment-merchant-details");
  ...
  var merchantImageNode = document.getElementById("hyper-checkout-merchant-image");
...
function renderDynamicMerchantDetails(paymentDetails) {
  ...
  var merchantDynamicDetails = document.getElementById("hyper-checkout-payment-merchant-dynamic-details");
...
function appendMerchantDetails(paymentDetails, merchantDynamicDetails) {
  ...
  if (!(Array.isArray(paymentDetails.transaction_details) && paymentDetails.transaction_details.length > 0)) {
  ...
  try {
    ...
    /* sort the merchant details based on the position, if position is null then it will be shown at the end */
...
function renderCart(paymentDetails) {
  ...
  if (Array.isArray(orderDetails) && orderDetails.length > 0) {
    ...
    var cartNode = document.getElementById("hyper-checkout-cart"),
        cartItemsNode = document.getElementById("hyper-checkout-cart-items"),
        MAX_ITEMS_VISIBLE_AFTER_COLLAPSE = paymentDetails.max_items_visible_after_collapse,
        yourCartText = document.createElement("span"),
        yourCartText = document.getElementById("your-cart-text");
    ...
    orderDetails.map(function (item, index) {
      ...
      renderCartItem(item, paymentDetails, index !== 0 && index < MAX_ITEMS_VISIBLE_AFTER_COLLAPSE, /* @ts-ignore */ cartItemsNode);
    ...
    if (totalItems > MAX_ITEMS_VISIBLE_AFTER_COLLAPSE) {
      ...
      var hiddenItemsCount = (orderDetails.length - MAX_ITEMS_VISIBLE_AFTER_COLLAPSE);
...
**/
function renderCartItem(item, paymentDetails, shouldAddDividerNode, cartItemsNode) {
  
  /* Wrappers */
  var itemWrapperNode = document.createElement("div"),
      nameAndQuantityWrapperNode = document.createElement("div"),
      productImageNode = document.createElement("img"),
      productNameNode = document.createElement("div");
  ...
  itemWrapperNode.append(productImageNode, nameAndQuantityWrapperNode, priceNode);
...
function handleCartView(paymentDetails) {
  ...
  var orderDetails = paymentDetails.order_details,
      MAX_ITEMS_VISIBLE_AFTER_COLLAPSE = paymentDetails.max_items_visible_after_collapse,
      itemsHTMLCollection = document.getElementsByClassName("hyper-checkout-cart-item"),
      dividerHTMLCollection = document.getElementsByClassName("hyper-checkout-cart-item-divider"),
      cartItems = [].slice.call(itemsHTMLCollection),
      dividerItems = [].slice.call(dividerHTMLCollection),
      isHidden = cartItems.length < orderDetails.length,
      cartItemsNode = document.getElementById("hyper-checkout-cart-items"),
      cartButtonTextNode = document.getElementById("hyper-checkout-cart-button-text"),
      cartButtonImageNode = document.getElementById("hyper-checkout-cart-button-arrow");
  ...
  if (isHidden) {
    ...
    if (Array.isArray(orderDetails)) {
      ...
      orderDetails.map(function (item, index) {
        ...
        renderCartItem(item, paymentDetails, index >= MAX_ITEMS_VISIBLE_AFTER_COLLAPSE, cartItemsNode);
    ...
    if (cartButtonImageNode instanceof Object && arrowUpImage instanceof Object) {
    ...
    setTimeout(function () {
      ...
      if (cartButtonImageNode instanceof Object && arrowDownImage instanceof Object) {
...
function renderSDKHeader(paymentDetails) {
  ...
  var sdkHeaderItemNode = document.createElement("div"),
      sdkHeaderMerchantNameNode = document.createElement("div"),
      sdkHeaderAmountNode = document.createElement("div");
