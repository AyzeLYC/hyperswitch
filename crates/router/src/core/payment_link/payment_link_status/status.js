...
/* @ts-ignore */
window.state = { prevHeight: window.innerHeight, prevWidth: window.innerWidth, isMobileView: window.innerWidth <= 1400 };
...
function renderStatusDetails(paymentDetails) {
  ...
  var status = paymentDetails.status,
      ...
      statusDetails = { imageSource: "", message: "", status: status, amountText: "", items: [] };
  ...
  switch (status) {
    ...
    case "failed":
      ...
      var unifiedErrorCode = paymentDetails.unified_code || paymentDetails.error_code,
          unifiedErrorMessage = paymentDetails.unified_message || paymentDetails.error_message,
          errorCodeNode = createItem(translations.errorCode, unifiedErrorCode),
          errorMessageNode = createItem(translations.errorMessage, unifiedErrorMessage);
    ...
    default:
      ...
      if (typeof paymentDetails.error === "object") {
        ...
        var errorCodeNode = createItem(translations.errorCode, paymentDetails.error.code),
            errorMessageNode = createItem(translations.errorMessage, paymentDetails.error.message);
  ...
  var statusImageNode = document.createElement("img"),
      ...
      statusMessageNode = document.createElement("div"),
      statusDetailsNode = document.createElement("div");
  ...
  var statusHeaderNode = document.getElementById("hyper-checkout-status-header);
  ...
  var statusContentNode = document.getElementById("hyper-checkout-status-content");
  ...
  if (paymentDetails.redirect === true) {
    ...
    /* Form redirect text */
    var statusRedirectTextNode = document.getElementById("hyper-checkout-status-redirect-message");
    
    if (statusRedirectTextNode instanceof HTMLDivElement && typeof paymentDetails.return_url === "string") {
      ...
      for (var i = 0; i <= timeout; i++) {
        ...
        setTimeout(function () {
          ...
          var secondsLeft = timeout - j++,
              innerText = secondsLeft === 0 ? translations.redirecting : translations.redirectingIn + secondsLeft + " " + translations.seconds;
          ...
          if (secondsLeft === 0) {
            ...
            /* Form query params */
            var queryParams = { payment_id: paymentDetails.payment_id, status: paymentDetails.status },
...
function initializeEventListeners(paymentDetails) {
  ...
  var primaryColor = paymentDetails.theme,
      ...
      statusRedirectTextNode = document.getElementById("hyper-checkout-status-redirect-message");
