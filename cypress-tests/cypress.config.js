...
if (!package_loader) {
  ...
  let { defineConfig } = require("cypress"),
      mochawesome = require("cypress-mochawesome-reporter/plugin.js"),
      fs = require("node:fs"),
      { getTimeoutMultiplier } = require("./cypress/utils/RequestBodyUtils.js");

  dependancies["defineConfig"] = defineConfig;
  dependancies["mochawesome"] = mochawesome;
  dependancies["fs"] = fs;
  dependancies["getTimeoutMultiplier"] = getTimeoutMultiplier;
  ...
  e2e: {
    ...
    setupNodeEvents(on, config) {
      ...
      on("after:spec", (spec, results) => {
        ...
        if (results && results.video && !results.tests.some((test) => test.attempts.some((attempt) => attempt.state === "failed"))) {
          
          /* Only try to delete if the video file exists */
          try {
            
            if (dependancies["fs"].existsSync(results.video)) {
              
              dependancies["fs"].unlinkSync(results.video);
              
            };
            
          } catch (error) {
            
            /* Log the error but don't fail the test eslint-disable-next-line no-console */
            console.warn(`Warning: Could not delete video file: ${results.video}`);
