...
var palette = { bold: 1, faint: 2, red: 31, green: 32, cyan: 36/*TODO: add others?*/ };

var groupPrefix = '█',
    detailsPrefix = '↳',
    succMark = '✓',
    failMark = '✗',
    defaultOptions = {indent: ' ', enableColors: true, summaryTimeUnit: null, summaryTrendStats: null}
...
function summarizeCheck(indent, check, decorate) {
  ...
  return decorate(`${indent}${failMark} ${check.name}\n${indent} ${detailsPrefix}  ${succPercent}% — ${succMark} ${check.passes} / ${failMark} ${check.fails}`, palette.red);
...
function humanizeBytes(bytes) {
  ...
  var e = Math.floor(Math.log(bytes) / Math.log(base)),
      suffix = units[e | 0],
      val = Math.floor((bytes / Math.pow(base, e)) * 10 + 0.5) / 10;
  
  return val.toFixed(val < 10 ? 1 : 0) + ' ' + suffix;
  
};

var unitMap = { s: { unit: 's', coef: 0.001 }, ms: { unit: 'ms', coef: 1 }, us: { unit: 'µs', coef: 1000 }, };
...
function nonTrendMetricValueForSum(metric, timeUnit) {
  ...
  switch (metric.type) {
    ...
    case 'counter':
      ...
      return [ humanizeValue(metric.values.count, metric, timeUnit), humanizeValue(metric.values.rate, metric, timeUnit) + '/s', ];
      
    case 'gauge':
      
      return [ humanizeValue(metric.values.value, metric, timeUnit), 'min=' + humanizeValue(metric.values.min, metric, timeUnit), 'max=' + humanizeValue(metric.values.max, metric, timeUnit), ];
      
    case 'rate':
      
      return [ humanizeValue(metric.values.rate, metric, timeUnit), `${succMark} ${metric.values.passes}`, `${failMark} ${metric.values.fails}`, ];
...
function summarizeMetrics(options, data, decorate) {
  ...
  var indent = `${options.indent}  `,
      ...
      nonTrendExtras = {},
      nonTrendExtraMaxLens = [0, 0],

      trendCols = {},
      numTrendColumns = options.summaryTrendStats.length,
      trendColMaxLens = new Array(numTrendColumns).fill(0);
  ...
  names.sort(function (metric1, metric2) {
    ...
    var parent1 = metric1.split('{', 1)[0],
        parent2 = metric2.split('{', 1)[0],
        result = parent1.localeCompare(parent2);
    ...
    var sub1 = metric1.substring(parent1.length),
        sub2 = metric2.substring(parent2.length);
  ...
  var getData = function (name) {
    ...
    if (trendCols.hasOwnProperty(name)) {
      ...
      var cols = trendCols[(name)],
          tmpCols = new Array(numTrendColumns);
    ...
    var value = nonTrendValues[(name)],
        fmtData = decorate(value, palette.cyan) + ' '.repeat(nonTrendValueMaxLen - strWidth(value)),
  ...
  for (var name of names) {
    ...
    fmtName = `${fmtName}${decorate('.'.repeat(nameLenMax - strWidth(fmtName) - strWidth(fmtIndent) + 3) + ':', palette.faint)}`;
  ...
  forEach(data.metrics, function (metricName, metric) {
    ...
    forEach(metric.thresholds, function (thresholdName, threshold) {
      ...
      if (threshold.ok) {
        ...
        failures++;
        cases.push(`<testcase name="${escapeHTML(metricName)} - ${escapeHTML(thresholdName)}"><failure message="failed" /></testcase>`);
